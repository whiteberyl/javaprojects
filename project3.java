...
public class project3 { 
   public static void main(String[] args) {
         ...
         if (!textScanner.hasNextInt()) { // data type check
            System.out.println("Error: Non-intiger input.");
            System.exit(0);
         }
         ...
         //Input 3 2D array Part 1-5 ask for line 0, 1, 2 no error checking
         ...
         for (int = 0; 1 < paragraph.length; i++) {
         ...
         //Calculation 3 alphabetical rearrangement; tripple for loop, j and k (lecture 5 hint)
         for (int i = 0; i < paragraph.length; i++) {
            for (int j = 0; j < paragraph[i].length - 1; j++) {
               for (int k = j + 1; k < paragraph[i].length; k++) {
                  for (paragraph[i][j].compareToIgnoreCase(paragraph[i][k]) > 0 {
                     String temporary = paragraph[i][j];
                     paragraph[i][j] = parragraph[i][k]'
